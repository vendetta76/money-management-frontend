...
const IncomePage = () => {
  ...
  useEffect(() => {
    ...
    const unsubIncomes = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map((doc) => {
        const entry = doc.data() as IncomeEntry;
        return {
          id: doc.id,
          wallet: entry.wallet || "",
      ...
      setIncomes(data);
      console.log("Incomes updated:", data); // Debug logging
    }, (error) => {
    ...
    const unsubWallets = onSnapshot(walletRef, (snapshot) => {
      const data = snapshot.docs.map((doc) => {
        const wallet = doc.data() as WalletEntry;
        return {
          id: doc.id,
      ...
      setWallets(data);
      console.log("Wallets updated:", data); // Debug logging
    }, (error) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "amount") {
      // Format amount like OutcomePage.tsx
  ...
  const validate = () => {
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) newErrors.amount = "Nominal harus lebih dari 0.";
                  ...
                  >
                    <div className="text-sm">
                      ...
                      <div className="text-gray-500 dark:text-gray-400">
                        {(wallets.find((w) => w.id === entry.wallet)?.name || entry.wallet) + " (Telah dihapus)"} · {entry.currency}
                      </div>
